...
| 模块 | 说明 |
|------|------|
| 翻译 | 流式输出；`⌘↩` 发送；语言方向可选；**语气预设**（标准/正式/口语/技术文档） |
| 引擎 | DeepSeek / 小米 MiMo / 智谱 GLM / 自定义 OpenAI 兼容接口 |
| 模型 | 翻译页徽章快速切换；设置里可手动填 ID 或从 `/models` 拉取列表 |
| 快捷键 / 划词 | 双击 `⌘` 打开空白快速翻译框；**任意处选中文字**后光标旁出现「翻译」按钮，点击即译并在快速框展示 |
| 右键服务 | —（已移除系统「服务」菜单项） |
...
| 入口 | 行为 |
|------|------|
| 双击 `⌘` | 弹出**空白**快速翻译框：自己粘贴或输入 → 点「翻译」 |
| 选中文字 | 系统任意处选中（≥2 字）→ 光标旁出现 **「翻译」** 按钮 → 点击后自动翻译并在快速框展示 |

划词需要在 **系统设置 → 隐私与安全性 → 辅助功能** 中允许 Sakura Translator（应用内设置页可检查状态）。

窗口为无边框浮层，**没有**系统关闭/最小化/全屏按钮；关闭请点框内右上角 **✕**，或再次双击 `⌘`。
